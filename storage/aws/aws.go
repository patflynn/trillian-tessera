...
//
// Deprecated: use [github.com/transparency-dev/tessera/storage/objstore]. This
// package is a compatibility wrapper that opens an s3blob bucket and delegates.
package aws
...
// DefaultIntegrationSizeLimit is the default maximum number of entries to
// integrate in a single call.
//
// Deprecated: use objstore.DefaultIntegrationSizeLimit.
const DefaultIntegrationSizeLimit = objstore.DefaultIntegrationSizeLimit

// SchemaCompatibilityVersion represents the expected version (e.g. layout &
// serialisation) of stored data.
//
// Deprecated: use objstore.SchemaCompatibilityVersion.
const SchemaCompatibilityVersion = objstore.SchemaCompatibilityVersion
...
// gocloud.dev/blob bucket instead of AWS SDK configuration.
type Config struct {
...
//
// Deprecated: use objstore.New with an s3blob-backed bucket. This wrapper builds
// the S3 client from cfg and delegates to objstore.New.
func New(ctx context.Context, cfg Config) (tessera.Driver, error) {
	...
	// This wrapper owns bkt but objstore exposes no Close, so it stays open for
	// the process lifetime. Fine for long-lived servers; code creating many
	// short-lived instances should use objstore.New with a caller-owned bucket.
