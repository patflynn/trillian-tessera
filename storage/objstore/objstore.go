...
//
// The bucket is provided via the portable gocloud.dev/blob abstraction (see
// blob.go), so the same code serves GCS, any S3-compatible store, the local
// filesystem or an in-memory bucket. The application opens the bucket and passes
// it via Config.Bucket, keeping provider driver dependencies out of this library.
//
...
// newObjStore wraps Config.Bucket in the blob-backed object store (see blob.go).
...
func (a *Appender) updateEntryBundles(ctx context.Context, fromSeq uint64, entries []storage.SequencedEntry) error {
	...
	for _, e := range entries {
		...
		if entriesInBundle == layout.EntryBundleWidth {
			...
			slog.DebugContext(ctx, "In-memory bundle is full, attempting write to object store", slog.Uint64("bundleindex", bundleIndex))
	...
	if entriesInBundle > 0 {
		slog.DebugContext(ctx, "Attempting to write in-memory partial bundle idx . to object store", slog.Uint64("bundleindex", bundleIndex), slog.Uint64("entriesinbundle", entriesInBundle))
...
func (s *mySQLSequencer) publishCheckpoint(ctx context.Context, minStaleActive, minStaleRepub time.Duration, f func(context.Context, uint64, []byte) error) (nextPubAt time.Time, errR error) {
	...
	var pubAt time.Time
	// publishedAt is nanoseconds; older rows may be seconds.
...
func (s *mySQLSequencer) garbageCollect(ctx context.Context, treeSize uint64, maxBundles uint, deleteWithPrefix func(ctx context.Context, prefix string) error, entriesPath func(uint64, uint8) string) error {
	// number of concurrent object store deletes to perform while collecting garbage.
	const numWorkers = 5
	tx, err := s.dbPool.Begin()
	if err != nil {
		return err
