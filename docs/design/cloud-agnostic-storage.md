...
**Status:** Proposal · **Scope:** object-storage backend for Tessera
...
|---|---|---|
| Object access | `objStore` interface → `s3Storage` (aws-sdk-go-v2) — as implemented, superseded by `blobStore` (`gocloud.dev/blob`, see `blob.go`) | **Interface: no.** Impl: yes |
...
unless shown to have no detrimental effect on AWS performance." → **minimize the diff to existing
files** so the change stays easy to review and rebase against a fast-moving codebase.
...
- **Strictly additive within `storage/objstore`:** new file(s) for the GCS impl + one constructor branch.
  The AWS path is byte-for-byte unchanged at runtime. **No package rename** for now (a rename is a
  separate, larger change; renaming up front maximizes rebase pain — see S3).

**Non-goals**
- Replacing/merging the existing `storage/gcp` (Spanner) backend.
- Up-front package renaming/restructuring — revisit once the backend has proven itself in practice.
...
5. **🟠 Naming smell** — a GCS-native file inside `storage/objstore` is confusing, and `storage/gcp`
   already means GCS+Spanner. Tolerate for now (minimal diff); only resolve via rename if/when the
   package layout is revisited.
...
- `objStore` interface — `storage/objstore/objstore.go`; its sole implementation
  is now `blobStore` (`gocloud.dev/blob`) in `storage/objstore/blob.go`. The
  original `s3Storage` (aws-sdk-go-v2) impl referenced throughout this proposal
  was superseded by `blobStore` when Option B landed (see "Implemented" note above).
